"""
09_5gnidd_generalizability.py
5G-NIDD Dataset — Generalizability Analysis
TabNet + AGE (Transfer) + PSRI on 5G-NIDD

Çalıştırma: python 09_5gnidd_generalizability.py
"""
...
# ── 5G DİLİM HARİTASI — 5G-NIDD ─────────────────────────────────────────────
# Sınıflar: Benign, Nmap, Hping3, Goldeneye, Slowloris, Torshammer
# Hping3  → yüksek hacimli flood → eMBB
# Goldeneye, Slowloris, Torshammer → gecikme/güvenilirlik → URLLC
# Nmap → keşif/tarama → mMTC
SLICE_MAP = {
    "eMBB":   ["udpflood", "icmpflood", "httpflood", "synflood"],
    "URLLC":  ["slowratedos", "tcpconnectscan"],
    "mMTC":   ["synscan", "udpscan"],
    "Benign": ["benign"],
}
CLASS_TO_SLICE = {cls: sl for sl, classes in SLICE_MAP.items() for cls in classes}

# ── 1. VERİ YÜKLE ────────────────────────────────────────────────────────────
print("\n5G-NIDD verisi yukleniyor...")

data_path = os.path.join("CICIOT2023", "combined.csv")
df = pd.read_csv(data_path, low_memory=False)

print(f"OK: {len(df):,} satır yüklendi")

label_col = "Attack Type"

# Kategorik ve gereksiz sütunları çıkar
exclude = ["Unnamed: 0", "Seq", "Proto", "Cause", "State",
           "sDSb", "dDSb", "Label", "Attack Type", "Attack Tool"]

# Proto ve State encode et
cat_encode = ["Proto", "Cause", "State", "sDSb", "dDSb"]
...
print(f"OK: {len(feature_cols)} ozellik secildi")

# Label normalize et
df[label_col] = df[label_col].str.strip().str.lower()

# Örnekleme — 200K yeterli
if len(df) > 200000:
    df = df.groupby(label_col, group_keys=False).apply(
        lambda x: x.sample(min(len(x), int(200000 * len(x)/len(df))), random_state=42)
    ).reset_index(drop=True)
    print(f"OK: {len(df):,} satıra örneklendi")
...
print("Sinif dagilimi (test):")
for cls in le.classes_:
    cnt = (y_test_r == cls).sum()
    sl  = CLASS_TO_SLICE.get(cls, "Unknown")
    print(f"   {cls:<15} {cnt:>7,}  → {sl}")

print("\nDilim dagilimi (test):")
for sl in ["eMBB","URLLC","mMTC","Benign"]:
    cnt = (slice_labels == sl).sum()
    print(f"   {sl:<8} {cnt:>7,}  ({cnt/len(X_test)*100:.1f}%)")

# ── 2. TABNET EĞİTİMİ ────────────────────────────────────────────────────────
print("\nTabNet egitiliyor (5G-NIDD)...")
t0 = time.time()
...
print(f"OK: TabNet 5G-NIDD — Acc={acc_clean:.4f} F1_w={f1_clean:.4f} F1_mac={f1_mac_clean:.4f} ({elapsed:.0f}s)")
...
top_feat_idx = []
for feat in feat_imp.head(5).index:
    if feat in feature_cols:
        top_feat_idx.append(feature_cols.index(feat))
...
for sl in slices:
    ...
    if mask.sum() == 0:
        print(f"  UYARI: {sl} diliminde ornek yok, atlaniyor")
        continue
...
# ── 7. RAPOR ─────────────────────────────────────────────────────────────────
print("\n" + "="*70)
print("5G-NIDD PSRI SONUÇLARI (ε=0.10)")
print("="*70)
...
ax.set_title("5G-NIDD — Per-Slice PSRI", fontweight="bold")
...
ax = axes[1]
ciciot_psri_age  = {"eMBB": 0.488, "URLLC": 0.023, "mMTC": 0.983, "Benign": 1.000, "GLOBAL": 0.444}
ciciot_psri_fgsm = {"eMBB": 0.603, "URLLC": 0.022, "mMTC": 0.453, "Benign": 1.006, "GLOBAL": 0.502}

ciciot_age_v  = [ciciot_psri_age.get(s, 0)  for s in slice_order]
ciciot_fgsm_v = [ciciot_psri_fgsm.get(s, 0) for s in slice_order]

x2 = np.arange(len(slice_order))
w2 = 0.2
ax.bar(x2-w2*1.5, ciciot_age_v,  w2, label="CICIoT — AGE",    color="#E74C3C", alpha=0.85)
ax.bar(x2-w2*0.5, ciciot_fgsm_v, w2, label="CICIoT — FGSM",   color="#3498DB", alpha=0.85)
ax.bar(x2+w2*0.5, psri_age_v,    w2, label="5G-NIDD — AGE",   color="#E74C3C", alpha=0.4, hatch="//")
ax.bar(x2+w2*1.5, psri_fgsm_v,   w2, label="5G-NIDD — FGSM",  color="#3498DB", alpha=0.4, hatch="//")
ax.axhline(y=1.0, color="gray", linestyle="--", alpha=0.5)
ax.set_xticks(x2); ax.set_xticklabels(slice_order, fontsize=10)
ax.set_ylabel("PSRI"); ax.set_ylim(0, 1.2)
ax.set_title("CICIoT2023 vs 5G-NIDD — PSRI Comparison", fontweight="bold")
ax.legend(fontsize=8); ax.grid(True, alpha=0.3, axis="y")

plt.tight_layout()
plt.savefig("reports/13_5gnidd_comparison.png", dpi=150, bbox_inches="tight")
plt.close()
print("\nOK: reports/13_5gnidd_comparison.png")

pd.DataFrame([
    {"Dataset": "5G-NIDD", "Dilim": sl,
     "N": results[sl]["n"],
...
]).to_csv("reports/5gnidd_psri_sonuclari.csv", index=False)
print("OK: reports/5gnidd_psri_sonuclari.csv")
